...
# Mapear email → nome no JSON de autoavaliacao
with open("/opt/data/convenia/autoavaliacao_perguntas.json") as f:
    auto_data = json.load(f)

# Carregar lider_data (pode ser None se nao existir)
lider_data = None

# Encontrar colaborador - busca flexivel
colaborador = None
tipo_form = "autoavaliacao"
source_data = auto_data

# Normalizar email para comparacao
email_parts = EMAIL.lower().replace("@condoconta.com.br","").replace("@","").split(".")
email_first = email_parts[0] if email_parts else ""
...
for t, src, col in all_cols:
    ...
    # Match exato de email
    if EMAIL.lower() == col.get("email", "").lower():
        best = (t, src, col)
        break
    
    # Match por partes do nome vs email
...
instrumento = source_data["instrumento"]
